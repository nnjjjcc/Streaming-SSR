...
export function stubTransport(options) {
    return {
        unary(service, method, signal, timeoutMs, header, message) {
            return runUnary(createUnaryRequest(options, service, method, signal, header, message), (req) => Promise.resolve(createUnaryResponse(options, service, method, req)), options.interceptors);
        },
        stream(service, method, signal, timeoutMs, header, input) {
            return runStreaming(createStreamingRequest(options, service, method, signal, header, input), (req) => Promise.resolve(createStreamingResponse(options, req)), options.interceptors);
        },
    };
}
/**
