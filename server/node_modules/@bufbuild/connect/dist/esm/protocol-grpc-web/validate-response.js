...
import { headerContentType, headerEncoding, headerGrpcMessage, headerGrpcStatus, } from "./headers.js";
/**
 ...
 */
export function validateResponse(useBinaryFormat, status, headers) {
    var _a;
    const code = codeFromHttpStatus(status);
    if (code != null) {
        throw new ConnectError(decodeURIComponent((_a = headers.get(headerGrpcMessage)) !== null && _a !== void 0 ? _a : `HTTP ${status}`), code);
    }
    const mimeType = headers.get(headerContentType);
    const parsedType = parseContentType(mimeType);
    if (!parsedType || parsedType.binary != useBinaryFormat) {
        throw new ConnectError(`unexpected response content type ${mimeType !== null && mimeType !== void 0 ? mimeType : "?"}`, Code.Internal);
    }
    if (parsedType.text) {
        throw new ConnectError("grpc-web-text is not supported", Code.InvalidArgument);
    }
 ...
 * @private Internal code, does not follow semantic versioning.
 */
export function validateResponseWithCompression(useBinaryFormat, acceptCompression, status, headers) {
