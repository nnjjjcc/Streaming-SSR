...
function validateUniversalHandlerOptions(opt) {
    ...
    const maxDeadlineDurationMs = (_b = opt.maxDeadlineDurationMs) !== null && _b !== void 0 ? _b : Number.MAX_SAFE_INTEGER;
    return Object.assign(Object.assign({ acceptCompression }, (0, limit_io_js_1.validateReadWriteMaxBytes)(opt.readMaxBytes, opt.writeMaxBytes, opt.compressMinBytes)), { jsonOptions: opt.jsonOptions, binaryOptions: opt.binaryOptions, maxDeadlineDurationMs, shutdownSignal: opt.shutdownSignal, requireConnectProtocolHeader });
}
...
function negotiateProtocol(protocolHandlers) {
    ...
    async function protocolNegotiatingHandler(request) {
        ...
        const matchingContentTypes = protocolHandlers.filter((h) => h.supportedContentType(contentType));
        if (matchingContentTypes.length == 0) {
            return universal_js_1.uResponseUnsupportedMediaType;
        }
        const matchingMethod = matchingContentTypes.filter((h) => h.allowedMethods.includes(request.method));
